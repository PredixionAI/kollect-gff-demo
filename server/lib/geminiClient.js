...
Output EXACTLY 5 lines, in this exact order, nothing before or after, one field per line, plain text only, no markdown, no bullet points:
...
WHATSAPP: <a short natural follow-up message referencing what was actually discussed, matching the tone/language of "${language || 'Hinglish'}", max ${LIMITS.whatsappCopy} chars>

Do not pad any field. Do not repeat the borrower's name more than once total across all 5 lines. If the call didn't cover something (e.g. no promise-to-pay date mentioned), leave that reasoning out rather than speculating. Ground every claim in the transcript below — never invent a date, amount, or promise the transcript doesn't contain.
